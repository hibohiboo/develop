...
if [ $? -eq 0 ]; then
  # 一致したときの処理
  cd $bin_dir/../docker && docker-compose exec $container_name  sqlplus sys/oraoraora@localhost:1521/XE as sysdba
else
  # 一致しなかった時の処理
  # コンテナを立ち上げて接続
  ...
  cd $bin_dir/../docker && docker-compose exec $container_name  sqlplus sys/oraoraora@localhost:1521/XE as sysdba  > /dev/null 2>&1
