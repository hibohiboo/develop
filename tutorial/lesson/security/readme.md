...
権限マトリックスを用意するとよい。
例えば、SaaS(Software as a Service)型のアプリケーションを想定する
...
#### ロールとは
上記表の、「システム管理者」「企業管理者」「一般ユーザ」はロール(role)と呼ばれるもの。
ロールとは、権限を組み合わせて役割を示す名前をつけたもの。
### 認可制御の正しい実装
* この機能を実行（画面を表示）してよいユーザであるか
* リソースに対する操作（参照・変更・削除など）の権限はあるか

ユーザ情報は外部から書換のできないセッション変数に保存する
...
### ログの種類
* Webサーバ（Apache,nginx,IISなど）のログ
* アプリケーションログ
* データベースログ

アプリケーションログについて、さらに分類する。
* エラーログ
* アクセスログ
* デバッグログ

#### エラーログ
* 利用者には「しばらく待ってからお試しください」のように表示し、エラーの詳しい内容や原因はログに出力するようにする。
  * 利用者にシステムのエラーを出しても困惑するだけ
  * エラーの内容が攻撃者へのヒントになるため
* 攻撃の検出に役立つ場合がある。SQLインジェクションやディレクトリ・トラバーサルの試行中にはSQLやファイルオープンのエラーが出やすくなる。

#### アクセスログ
* 正常・異常ともにログに残す
* 個人情報漏洩・事故への対応を通じて、正常系のログの重要性が認識されはじめた
* 各種の法令やガイドラインでも要求されている

#### デバッグログ
* ログのデータ量が膨大になり、パフォーマンスに影響が出る場合もある。
* 開発環境やテスト環境で取得すべきものであり、本番環境では取得するものではない
* 個人情報やクレジットカード情報が流出した例がある
...
### ログの出力項目
4W1H(いつ、誰が、どこで、何を、どのように)に従った以下の項目
...
* 操作内容（閲覧、変更、削除など）
* 操作対象（リソースIDなど）
* 捜査結果（成功あるいは失敗、処理件数など）

### ログの保護
ログが改ざん・削除されると目的が達成できなくなる。
ログに対する不正アクセスができないようにする必要がある。
ログファイル自体に個人情報など機密情報を含めないようにするか、項目を省略できない場合はマスク処理が必要。
ログにパスワードやクレジットカード情報を含めてはならない。

### ログの出力先
可能であればログを保存するサーバはDBサーバやWebサーバとはべつにすべき。

### ログの保管期間
運用ルールとしてWebサイトの特性にあわせて決める

### サーバの時刻合わせ
単独で存在するものではなく、Webサーバー、アプリケーションサーバー、データベース、メールなど様々なログを組み合わせて調査する。
ログを突き合わせるために、各サーバーの時刻を同期させる必要がある。
ntpプロトコルがよく使われる。
